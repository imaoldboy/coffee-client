...
tools.isPowerOff(deviceList);
/*while(tools.isPowerOff(deviceList)==true){
	console.log("machine is power off, so sleep 10 seconds.");
	sleep.sleep(10);
}*/

var machinePowerStatus="unknown";
tools.eventPipe.on('machinePowerStatus', (powerStatus)=>{
	if(powerStatus == "error"){
		sleep.sleep(10);
		tools.reboot();
	}else if(powerStatus == "poweroff"){
		sleep.sleep(10);
		deviceList = tools.getDeviceList();
		if(machinePowerStatus == "unknown"){
			machinePowerStatus = "poweroff";
			tools.isPowerOff(deviceList);
		}
	}else if(powerStatus == "ok"){
		console.log("machine's power status is ok.");
		if(machinePowerStatus == "unknown"){
			machinePowerStatus = "ok";
			goCoffee();
		}
	}else{
		console.log("unknown machine's power status." + powerStatus);
	}
});

//main process to work coffee machine.
function goCoffee(){
	var readPort = null;
	var writePort = null;
...
function testMachineIsError(line){
